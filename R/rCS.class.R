...
setMethod("initialize", "rCS",
          function(.Object, x, exposure, exact, title)
          {
            ...
            .Col1Label = ifelse(title=="", "CS", title);
...
setMethod("show" ,"rCS" ,
  function(object){
    ...
    ec.xtable(object@rCS, align=align)
  }
)

# -----------------------------------------------------------------------------
# function: rCS (call real constructor)
# Return: an object of type rCS
# -----------------------------------------------------------------------------
rCS <- function(x, exposure, exact=FALSE, title="")
{
  return(new("rCS", x=x, exposure=exposure, exact=exact, title=title));
}
