...
# method show
# -----------------------------------------------------------------------------
setMethod("show" ,"CSTable" ,
  function(object) {
    digits =  c(0,0,0,0,1,0,0,1,2,3,3,4);
    align  =  c("l","r","c","c","r","c","c","r","r","r","r","r");
    ec.xtable(object@CSTable,  digits=digits, align=align, caption=object@caption);
  }
)

# -----------------------------------------------------------------------------
# function: CSTable (call real constructor)
# Return: an object of type CSTable
# -----------------------------------------------------------------------------
CSTable <- function(x, exposure=list(), exact=FALSE, rr=FALSE, ar=FALSE, pvalue=TRUE, caption="CSTable")
{
  return(new("CSTable", x=x,
             exposure=exposure,
             exact=exact,
             rr=rr,
             ar=ar,
